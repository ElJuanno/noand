...
<style>
:root{
  --brand:#2b8a3e; --bg:#edf7ef; --card:#fff; --line:#e7f0ea; --muted:#6a8b74; --ink:#23452d;
}
...
.tile{ background:var(--card); border:1px solid var(--line);
       border-radius:16px; box-shadow:0 6px 18px rgba(43,138,62,.06); padding:18px }
...
.badge-soft{ background:#f3faf5; color:var(--brand); border:1px solid var(--line);
...
.item input:checked + label{
  ...
  background:#f3faf5;
  color:var(--brand);
  box-shadow:0 4px 12px rgba(43,138,62,.12);
}
.item label:hover{
  border-color:var(--brand); background:#f9fdf9;
}
</style>
<br>
<div class="wrap">
  ...
  <form action="{{ route('alergias.store') }}" method="POST" class="tile">
    ...
    <div class="grid">
      @foreach($todas as $a)
        <div class="item">
          <input type="checkbox" id="alergia{{ $a->id }}" name="alergias[]" value="{{ $a->id }}"
                 @checked(in_array($a->id, $seleccionadas)) />
          <label for="alergia{{ $a->id }}">
            {{ $a->descripcion }}
          </label>
        </div>
      @endforeach
